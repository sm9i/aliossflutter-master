...
- (void)update:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if (oss == nil) {
        ...
        NSString *file = call.arguments[@"fileByte"];
       
        NSData *nsdataFromBase64String = [[NSData alloc] initWithBase64EncodedString:file options:0];
        OSSPutObjectRequest * put = [OSSPutObjectRequest new];
        // 必填字段
        put.bucketName = bucket;
        put.objectKey = key;
        //put.uploadingFileURL = [NSURL fileURLWithPath:file];
        put.uploadingData = nsdataFromBase64String;///<NSData *>; // 直接上传NSData
